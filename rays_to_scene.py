...
# that world position, or leave as None to use the auto top-down placement.
# CAM_LOOK_AT is where the camera points; None means scene center.
#
# World is [0,32]^3. Examples:
#   Top-down (auto):   CAM_POS = None
#   Top-down (manual): CAM_POS = np.array([16.0, 100.0, 16.0])  ; CAM_LOOK_AT = None
#   Front view:        CAM_POS = np.array([16.0,  16.0, 80.0])  ; CAM_LOOK_AT = np.array([16.0, 16.0, 16.0])
#   Corner angle:      CAM_POS = np.array([60.0,  50.0, 60.0])  ; CAM_LOOK_AT = np.array([16.0, 16.0, 16.0])
# =============================================================================
CAM_POS     = np.array([60.0, 50.0, 60.0])   # np.array([X, Y, Z]) or None
CAM_LOOK_AT = np.array([16.0, 16.0, 16.0])   # np.array([X, Y, Z]) or None
...
def choose_camera_and_light(bounds_min_world: np.ndarray, bounds_max_world: np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
    ...
    if CAM_POS is not None:
        ...
        # Auto: straight above the top of the bounding box
        dist    = max(diag * 2.0, 40.0)
        cam_pos = np.array([center[0], bounds_max_world[1] + dist, center[2]], dtype=np.float64)
