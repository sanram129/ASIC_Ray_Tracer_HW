...
class VoxelLoader:
    ...
    def __init__(self, dut, clock, log_prefix="VoxelLoader"):
        ...
        self.dut = dut
        self.clock = clock
        self.log = logging.getLogger(f"cocotb.{log_prefix}")
        ...
        for z in range(grid_size):
            for y in range(grid_size):
                for x in range(grid_size):
                    ...
                    # Wait for acceptance
                    await RisingEdge(self.clock)
                    # Read load_ready in ReadWrite phase (no ReadOnly to avoid
                    # phase violation on the next iteration's write)
                ...
                if format_type == 0:
                    ...
                    for line in f:
                        ...
                        if len(parts) == 2:
                            ...
                            await RisingEdge(self.clock)
                            # Read load_ready in ReadWrite phase (no ReadOnly)
                            if self.load_ready.value:
                                voxels_loaded += 1
